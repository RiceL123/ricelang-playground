...
After a little tinkering to prevent unnecessary re-renders with [react's memoization](https://react.dev/reference/react/useMemo), the basic functionality was complete. Although all was well, I decided to sneak in some [jotai](https://jotai.org) for state management to remove the `useMemo` calls and the passing of props to child components because of how much fun I had with [zustand](https://zustand-demo.pmnd.rs/) (another state management system from [poimandres](https://pmnd.rs/)) in other projects I'd been working on. Migrating to [jotai](https://jotai.org) turned out to be quite lovely to learn with the atoms being quite close to React's `useState` and being able to move behaviour out of my components into regular `.ts` files. This allowed refactoring to be incremental making the transition very smooth. Overall, a lovely library.
